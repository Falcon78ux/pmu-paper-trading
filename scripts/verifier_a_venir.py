...
traite chaque outsider qualifie comme une opportunite INDEPENDANTE.

NOUVEAU (16 sept) : LIMITE D'EXPOSITION GLOBALE PAR CHEVAL.
NOUVEAU (17 sept) : ENTROPIE COMME 12e VARIABLE DE v1.10 (sigmoid).
CORRIGE (17 sept, soir) : PLAFOND D'EXPOSITION EN POURCENTAGE DE
BANKROLL COMBINEE (FRACTION_EXPOSITION_CHEVAL).

NOUVEAU (19 sept) : DEPLOIEMENT DU LOGIT CONDITIONNEL EN DOUBLE
LOGGING - suite a une decouverte majeure de la semaine (veille
mathematique sur l'econometrie du choix discret), le VRAI v1.10 (mise
reelle, toutes les strategies derivees : recalibre, D4, sniper,
place, favori, antifav, dutch, snipercombine, ecartfaible,
consensus_place, couple_harville) utilise desormais un LOGIT
CONDITIONNEL (softmax intra-course, McFadden/Benter, 9 variables
per-cheval, calculer_probas_conditionnel_course dans commun.py) -
valide cette semaine comme superieur au sigmoid sur TOUTES les
dimensions testees : Brier/log-loss (16/16 fenetres), ROI/pari (5x
superieur), croissance (+1510.9%/an contre +1013.1%/an), drawdown
(7.9% contre 27.2%), calibration verifiee saine (deciles proches de
la diagonale, seulement 5.8% de value bets sur petits champs).

L'ANCIEN v1.10 SIGMOID (12 variables, avec entropie,
modele_v110_production.json) RESTE CALCULE ET LOGGUE COMME REFERENCE
DE COMPARAISON UNIQUEMENT - ses value bets sont enregistres sous le
tag "v110_sigmoid_ref" dans paris_virtuels.csv, avec leur mise
THEORIQUE (calculee comme si elles etaient reelles, pour permettre
une comparaison directe des performances), MAIS CE TAG NE CORRESPOND
A AUCUN FICHIER bankroll_*.json - aucune vraie bankroll n'est jamais
affectee par ces entrees. C'est le sens du "double logging" decide
par l'utilisateur : les deux modeles tournent en parallele, un seul
mise reellement.

L'entropie (biais_hippodrome, nb_partants_course egalement) devient
CADUQUE pour le vrai v1.10 (ces 3 variables sont constantes par
...
NOUVEAU (23 sept) : FILTRE D'INCERTITUDE (methode delta, borne basse a
95% de confiance sur p) applique a v1.4, v1.5, v1.8, v1.4+Genealogie -
remplace le seuil EV brut par un seuil EV CONSERVATEUR (calcule sur la
borne basse de p, pas le point central) pour ces 4 modeles. Valide le
23 sept par walk-forward sur donnees historiques : gain net sur ces 4
modeles (IC95% du retour moyen plus etroit et plus eloigne de zero) ;
teste et REJETE pour 2e favori (aggrave son IC - population
d'entrainement trop petite). Retro-compatible : si le fichier modele
n'a pas encore les champs "cov_params"/"cov_params_ordre" (avant mise
a jour), le filtre est desactive automatiquement et le comportement
reste identique a avant (seuil EV brut).

NOUVEAU (23 sept, suite) : KELLY SIMULTANE applique a v1.4, v1.5,
v1.4+Genealogie (pas v1.8, qui a une fraction Kelly variable selon
deferre_4_pieds - nuance non geree par cette premiere version) -
remplace le Kelly independant par cheval par une optimisation
numerique jointe sur TOUS les chevaux paries par CE modele dans CETTE
course (mutuellement exclusifs). Valide le 22-23 sept par walk-forward
: bankroll finale +4.6%, ROI +1.5pt sur 2656 courses testees, comparee
au plafond d'exposition ad hoc seul (qui reste actif ensuite, comme
garde-fou entre strategies differentes sur le meme cheval).
=============================================================================
"""
...
from commun import (
    ...
    calculer_proba_bas, calculer_proba_bas_v18, kelly_simultane_course,
    ...
    arrondir_mise_euro, MISE_MINIMUM, FRACTION_KELLY,
    obtenir_plafond_dynamique, PLAFOND_GAIN_PMU,
...
FRACTION_EXPOSITION_CHEVAL = 0.15
...
def appliquer_kelly_simultane(value_bets, bankroll):
    """NOUVEAU (23 sept) : remplace le Kelly independant par cheval
    (deja calcule dans chaque tuple de value_bets, indice 4) par une
    optimisation jointe sur TOUS les chevaux de CE modele dans CETTE
    course. value_bets : liste de tuples (cheval, cote, proba, ev,
    mise, ...) - le format exact de value_bets_v14/v15/v14sire (5
    elements, aucun element supplementaire). Retourne une nouvelle
    liste, meme format, avec la mise (indice 4) remplacee - les
    chevaux dont la mise recalculee tombe a 0 apres arrondi sont
    retires de la liste (comme le ferait mise>0 a la construction
    initiale)."""
    if len(value_bets) < 2:
        return value_bets
    picks = [(item[2], item[1]) for item in value_bets]  # (proba, cote)
    mises_brutes = kelly_simultane_course(picks, bankroll, FRACTION_KELLY)
    resultat = []
    for item, mise_brute in zip(value_bets, mises_brutes):
        cote_item = item[1]
        plafond_palier = obtenir_plafond_dynamique(bankroll)
        plafond_gain = PLAFOND_GAIN_PMU / cote_item
        mise_finale = arrondir_mise_euro(min(mise_brute, plafond_palier, plafond_gain))
        if mise_finale > 0:
            resultat.append((item[0], item[1], item[2], item[3], mise_finale) + item[5:])
    return resultat
...
def main():
    ...
    modele_v110_sigmoid_ref = charger_json(f"{RACINE}/modele_v110_production.json")
    modele_v110_conditionnel = charger_json(f"{RACINE}/modele_v110_conditionnel_production.json")
    ...
    # NOUVEAU (19 sept) : bankroll THEORIQUE, purement pour affichage/log
    # de reference du chemin sigmoid - ne correspond a AUCUN fichier
    # bankroll_*.json reel, jamais mise a jour par verifier_resultats.py
    bankroll_v110_sigmoid_ref = 1236.0
    ...
    bankrolls_par_modele = {
        ...
        # "v110_sigmoid_ref" volontairement ABSENT de cette table - ses
        # paris ne comptent jamais dans le plafond d'exposition reel
    }
    ...
    for course in courses:
        ...
        value_bets_v110_sigmoid_ref = []  # NOUVEAU : reference uniquement
        candidats_place = []
        toutes_probas_v110 = []
        partants_avec_cote = []
        ...
        for p in participants:
            ...
            if sf_avant is not None and driver_forme is not None:
                valeurs_v14 = {"speed_figure_avant_course": sf_avant, "log_cote": log_cote, "driver_forme": driver_forme}
                proba14, contrib14 = calculer_proba_avec_contributions(valeurs_v14, modele_v14)
                if proba14 is not None:
                    ev14 = proba14 * cote - 1
                    # NOUVEAU (23 sept) : filtre d'incertitude - seuil sur la
                    # borne basse a 95% de confiance sur p, pas sur le point
                    # central. Repli automatique sur ev14 si le modele n'a pas
                    # encore cov_params (retro-compatible).
                    proba14_bas = calculer_proba_bas(valeurs_v14, modele_v14)
                    ev14_bas = (proba14_bas * cote - 1) if proba14_bas is not None else ev14
                    if ev14_bas > SEUIL_EV:
                ...
                if sire_forme is not None:
                    ...
                    proba14sire, contrib14sire = calculer_proba_avec_contributions(valeurs_v14sire, modele_v14sire)
                    if proba14sire is not None:
                        ev14sire = proba14sire * cote - 1
                        # NOUVEAU (23 sept) : filtre d'incertitude, meme principe que v1.4
                        proba14sire_bas = calculer_proba_bas(valeurs_v14sire, modele_v14sire)
                        ev14sire_bas = (proba14sire_bas * cote - 1) if proba14sire_bas is not None else ev14sire
                        if ev14sire_bas > SEUIL_EV:
            ...
            if sf_avant is not None and driver_forme is not None and biais_hippo is not None:
                ...
                proba15, contrib15 = calculer_proba_avec_contributions(valeurs_v15, modele_v15)
                if proba15 is not None:
                    ev15 = proba15 * cote - 1
                    # NOUVEAU (23 sept) : filtre d'incertitude, meme principe que v1.4
                    proba15_bas = calculer_proba_bas(valeurs_v15, modele_v15)
                    ev15_bas = (proba15_bas * cote - 1) if proba15_bas is not None else ev15
                    if ev15_bas > SEUIL_EV:
            ...
            if sf_avant is not None and driver_forme is not None and biais_hippo is not None:
                ...
                proba18, contrib18 = calculer_proba_v18_avec_contributions(valeurs_v18, modele_v18)
                if proba18 is not None:
                    ev18 = proba18 * cote - 1
                    # NOUVEAU (23 sept) : filtre d'incertitude - fonction dediee
                    # v1.8 (formule differente de calculer_proba_bas generique)
                    proba18_bas = calculer_proba_bas_v18(valeurs_v18, modele_v18)
                    ev18_bas = (proba18_bas * cote - 1) if proba18_bas is not None else ev18
                    if ev18_bas > SEUIL_EV:
        ...
        # =================================================================
        # KELLY SIMULTANE (23 sept) - v1.4, v1.5, v1.4+Genealogie
        # UNIQUEMENT (pas v1.8, voir docstring de appliquer_kelly_simultane).
        # S'applique APRES que tous les participants de la course aient ete
        # traites (il faut TOUS les chevaux paries par le modele dans cette
        # course pour l'optimisation jointe), AVANT le plafond d'exposition
        # croise entre strategies (qui reste actif ensuite, inchange).
        # =================================================================
        value_bets_v14 = appliquer_kelly_simultane(value_bets_v14, bankroll_v14)
        value_bets_v15 = appliquer_kelly_simultane(value_bets_v15, bankroll_v15)
        value_bets_v14sire = appliquer_kelly_simultane(value_bets_v14sire, bankroll_v14sire)

        # =================================================================
        # SECONDE PASSE - 2 CHEMINS EN PARALLELE (double logging) :
        # A) CONDITIONNEL (softmax intra-course) - LE VRAI v1.10, mise
        #    reelle, alimente toutes les strategies derivees.
        # B) SIGMOID (avec entropie) - REFERENCE UNIQUEMENT, mise
        #    theorique jamais reellement engagee.
        # =================================================================

        # --- A) CONDITIONNEL : calcule TOUS les chevaux de la course EN UNE FOIS ---
        liste_pour_conditionnel = [
            (i, item["valeurs_communes"]) for i, item in enumerate(candidats_v110_stage1)
        ]
        resultats_conditionnel = calculer_probas_conditionnel_course(liste_pour_conditionnel, modele_v110_conditionnel)
        probas_conditionnel_par_index = dict(resultats_conditionnel)

        for i, item in enumerate(candidats_v110_stage1):
            if i not in probas_conditionnel_par_index:
                continue
            cheval = item["cheval"]
            cote = item["cote"]
            num_pmu_cheval = item["num_pmu"]
            deferre_4_pieds = item["deferre_4_pieds"]
            proba110 = probas_conditionnel_par_index[i]
        ...
        # --- B) SIGMOID (reference) : entropie puis proba 12 variables, comme avant ---
        ...
        for item in candidats_v110_stage1:
            ...
            proba110_sigmoid, contrib110 = calculer_proba_v110_avec_entropie_et_contributions(valeurs_avec_entropie, modele_v110_sigmoid_ref)
            if proba110_sigmoid is None:
                continue

            ev110_sigmoid = proba110_sigmoid * cote - 1
            if ev110_sigmoid > SEUIL_EV:
                mise110_sigmoid_theorique = calculer_mise_v110(proba110_sigmoid, cote, bankroll_v110_sigmoid_ref, deferre_4_pieds)
                if mise110_sigmoid_theorique > 0:
                    value_bets_v110_sigmoid_ref.append((cheval, cote, proba110_sigmoid, ev110_sigmoid, mise110_sigmoid_theorique))
        ...
        # NOUVEAU (19 sept) : le chemin sigmoid de reference n'est PAS
        # inclus dans les notifications Telegram (mise theorique
        # uniquement, aucune action a prendre par l'utilisateur) -
        # uniquement enregistre dans paris_virtuels.csv pour analyse.
        ...
        # UNIQUEMENT aux paris REELS (le sigmoid de reference est
        # ajoute au log APRES ce controle, plus bas, et n'y participe
        # jamais). NOTE (23 sept) : reste actif comme garde-fou CROISE
        # entre strategies differentes sur le meme cheval - le Kelly
        # simultane ci-dessus ne couvre que l'interieur d'UN SEUL
        # modele, ces deux mecanismes sont complementaires, pas
        # redondants.
        # ---------------------------------------------------------------
        ...
        # NOUVEAU (19 sept) : ajoute le chemin sigmoid de reference APRES
        # le controle d'exposition (mise theorique, jamais plafonnee ni
        # comptee dans l'exposition reelle des autres strategies).
        for cheval, cote, proba, ev, mise in value_bets_v110_sigmoid_ref:
            lignes_course.append({"race_id": race_id, "modele": "v110_sigmoid_ref", "cheval": cheval, "cote": cote, "cote_cloture": "", "ev": ev, "mise": mise, "date_detection": maintenant.isoformat()})
    ...
    print(f"Verification terminee. {len(log_paris)} value bets detectes (reels + reference).")
